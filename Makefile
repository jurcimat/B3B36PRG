...
GREP := grep
CC := clang 
CFLAGS+= -pedantic -Wall -g -Werror -Werror=unused-result -std=c99 -O3
...
optest: FORCE
 ...
 mkdir -p ${OPTESTOUTPUT} ; \
	...
	@$(foreach var,$(OPTESTS), ./program -prg-optional < data/opt/hw02-opt-${var}.in 1> ${OPTESTOUTPUT}TEST_${var} 2> /dev/null; if [ "`hexdump ${OPTESTOUTPUT}TEST_${var}`" = "`hexdump data/opt/hw02-opt-${var}.out`" ] ; then \
printf "OPTEST ${var} OUT [\033[0;32m OK \033[0m] "; \
else \
printf "\n ----- EXPECTED RESULT ----- \n\n" >> ${OPTESTOUTPUT}TEST_${var}; \
cat data/opt/hw02-opt-${var}.out >> ${OPTESTOUTPUT}TEST_${var}; \
printf "\n--- HEX ---\n" >> ${OPTESTOUTPUT}TEST_${var}; \
 ./program -prg-optional <data/opt/hw02-opt-${var}.in 2>/dev/null| hexdump -C >> ${OPTESTOUTPUT}TEST_${var} ; \
printf "\n--- EXPECTED HEX ---\n" >> ${OPTESTOUTPUT}TEST_${var}; \
hexdump -C data/opt/hw02-opt-${var}.out >> ${OPTESTOUTPUT}TEST_${var}; \
printf "OPTEST ${var} OUT [\033[0;31mFAIL\033[0m] "; \
fi;\
if  test -f data/opt/hw02-opt-${var}.err  ; then \
printf  " ------- [ STDERR ] ------\n"  >>${OPTESTOUTPUT}TEST_${var};\
 if [ "`./program -prg-optional  < data/opt/hw02-opt-${var}.in 2>&1 >/dev/null | tee -a ${OPTESTOUTPUT}TEST_${var} | hexdump`" = "`hexdump data/opt/hw02-opt-${var}.err`" ] ; then \
printf " ERR [ \033[0;32mOK \033[0m]";    \
else  \
printf "\n ----- EXPECTED RESULT ----- \n\n" >> ${OPTESTOUTPUT}TEST_${var}; \
cat data/opt/hw02-opt-${var}.err >> ${OPTESTOUTPUT}TEST_${var}; \
printf "\n--- HEX ---\n" >> ${OPTESTOUTPUT}TEST_${var}; \
./program -prg-optional < data/opt/hw02-opt-${var}.in 2>&1 | hexdump -C >> ${OPTESTOUTPUT}TEST_${var} ; \
printf "\n--- EXPECTED HEX ---\n" >> ${OPTESTOUTPUT}TEST_${var}; \
hexdump -C data/opt/hw02-opt-${var}.err >> ${OPTESTOUTPUT}TEST_${var};\
printf " ERR [\033[0;31mFAIL\033[0m]"; \
 fi; \
 fi; \
 if [ "${VALGRIND}" = "1" ]; then\
 valgrind --leak-check=full --error-exitcode=125 ./program -prg-optional < data/opt/hw02-opt-${var}.in >/dev/null 2>&1; \
RETURN=$$?;\
printf " VALGRIND";  \
if [ $$RETURN = 125 ] ; then\
 printf  " [\033[0;31mFAIL\033[0m]"; \
...
./program -prg-optional < data/opt/hw02-opt-${var}.in >/dev/null 2>&1;\
 RETURN=$$?;\
printf " PROGRAM RETURNED $$RETURN";  \
if  test -f data/opt/hw02-opt-${var}.code  ; then \
 if [ RETURN = "`cat data/opt/hw02-opt-${var}.code`" ] ; then\
 printf  " [ \033[0;32mOK \033[0m]";  \
 else \
 printf  " [\033[0;31mFAIL\033[0m]"; \
 fi;\
